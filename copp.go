...
func NewCOPParser(g *Grammar, src []byte, concurrency int, avgTokenLength int, strategy ReductionStrategy) *COPParser {
	p := &COPParser{
		g:                 g,
		...
		results: make([]*CyclicParserStack, concurrency),
	}
...
func (p *COPParser) Parse(ctx context.Context, tokensLists []*ListOfStacks[Token]) (*Token, error) {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()

	p.concurrency = len(tokensLists)

	resultCh := make(chan parseResult[*CyclicParserStack])
	...
	for p.concurrency--; p.concurrency >= 1; p.concurrency-- {
		if p.reductionStrategy == ReductionSweep || (p.reductionStrategy == ReductionMixed && reductionPasses >= 2) {
			...
			input := p.CombineSweepLOSCyclic(p.pools.sweepInput, p.results[1:])
...
func (p *COPParser) CombineSweepLOSCyclic(pool *Pool[stack[Token]], stacks []*CyclicParserStack) *ListOfStacks[Token] {
...
func (w *coppWorker) parse(ctx context.Context, stack *CyclicParserStack, tokens *ListOfStacks[Token], nextToken *Token, finalPass bool, resultCh chan<- parseResult[*CyclicParserStack], errCh chan<- error) {
	tokensIt := tokens.HeadIterator()

	prefix := make([]TokenType, w.p.g.MaxRHSLength)
	prefixTokens := make([]*Token, w.p.g.MaxRHSLength)
	...
	resultCh <- parseResult[*CyclicParserStack]{w.id, stack}
}
