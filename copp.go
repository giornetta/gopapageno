...
type COPParser[T Tokener] struct {
	g *Grammar[CToken]
	...
	// Pools
	pools struct {
		stacks       []*Pool[stack[*CToken]]
		nonterminals []*Pool[CToken]

		// These are only used when reducing using a single sweep.
		sweepInput *Pool[stack[CToken]]
		sweepStack *Pool[stack[*CToken]]
	}

	workers []*coppWorker[T]
	results []*COPPStack
}

func NewCOPParser[T Tokener](
	g *Grammar[CToken],
	src []byte, concurrency int, avgTokenLength int, strategy ReductionStrategy) *COPParser[T] {
	p := &COPParser[T]{
		g:                 g,
		concurrency:       concurrency,
		reductionStrategy: strategy,
		workers:           make([]*coppWorker[T], concurrency),
		results:           make([]*COPPStack, concurrency),
	}

	srcLen := len(src)
	stackPoolBaseSize := stacksCount[*CToken](src, p.concurrency, avgTokenLength)
	ntPoolBaseSize := srcLen / avgTokenLength / p.concurrency

	// Initialize memory pools for stacks.
	p.pools.stacks = make([]*Pool[stack[*CToken]], p.concurrency)

	// Initialize pools to hold pointers to tokens generated by the reduction steps.
	p.pools.nonterminals = make([]*Pool[CToken], p.concurrency)
	...
	if strategy == ReductionSweep || strategy == ReductionMixed {
		inputPoolBaseSize := stacksCount[CToken](src, p.concurrency, avgTokenLength)

		p.pools.sweepInput = NewPool(inputPoolBaseSize, WithConstructor(newStack[CToken]))
		p.pools.sweepStack = NewPool(stackPoolBaseSize, WithConstructor(newStack[*CToken]))
	}

	for thread := 0; thread < concurrency; thread++ {
		p.workers[thread] = &coppWorker[T]{
			p:      p,
			id:     thread,
...
func (p *COPParser[T]) Parse(ctx context.Context, tokensLists []*LOS[T]) (*T, error) {
	tokens := any(tokensLists).([]*LOS[CToken])
	...
	resultCh := make(chan workerResult[*COPPStack])
	errCh := make(chan error, 1)

	// First parallel pass of the algorithm.
	for thread := 0; thread < p.concurrency; thread++ {
		var nextToken *CToken
		...
		s := NewCOPPStack(p.pools.stacks[thread])
...
func (p *COPParser[T]) CombineSweepLOS(pool *Pool[stack[CToken]], stacks []*COPPStack) *LOS[CToken] {
	input := NewLOS(pool)

	tokenSet := make(map[*CToken]struct{}, stacks[0].Length())
	for i := 0; i < p.concurrency-1; i++ {
		it := stacks[i].HeadIterator()

		//Ignore the first token.
		t := it.Next()
		tokenSet[t] = struct{}{}
		for _, t := range stacks[i].StateTokenStack.Slice(t.State.CurrentIndex, t.State.CurrentLen) {
			tokenSet[t] = struct{}{}
		}
		...
		for t := it.Next(); t != nil; t = it.Next() {
			...
			for _, stateToken := range stacks[i].StateTokenStack.Slice(t.State.CurrentIndex, t.State.CurrentLen) {
				if _, ok := tokenSet[stateToken]; !ok {
					stateToken.Precedence = PrecEmpty
					input.Push(*stateToken)
...
type coppWorker[T Tokener] struct {
	p      *COPParser[T]
	id     int
	ntPool *Pool[CToken]
	// producedTokens maps the rightmost rhs-token of a prefix production with its parent.
	producedTokens map[*CToken]*CToken
}

// parseCyclic implements COPP.
func (w *coppWorker[T]) parse(ctx context.Context, stack *COPPStack, tokens *LOS[CToken], nextToken *CToken, finalPass bool, resultCh chan<- workerResult[*COPPStack], errCh chan<- error) {
	tokensIt := tokens.HeadIterator()

	prefix := make([]TokenType, w.p.g.MaxRHSLength)
	prefixTokens := make([]*CToken, w.p.g.MaxRHSLength)
	...
	for inputToken := tokensIt.Next(); inputToken != nil; {
		...
		if prec == PrecYields {
			...
			if lhs != TokenEmpty && w.p.g.Rules[ruleNum].Type != RuleSimple {
			...
			t := stack.Pop()
			inputToken.State = t.State
			stack.PushWithState(inputToken)
			...
			if !stack.IsYieldingPrecedence() {
				...
				// Prefix is made of a single nonterminal
				if t.State.CurrentLen == 1 && !stack.StateTokenStack.Data[t.State.CurrentIndex].IsTerminal() {
					stack.State.PreviousIndex = t.State.PreviousIndex
					stack.State.PreviousLen = t.State.PreviousLen
				} else {
					stack.State.PreviousIndex = t.State.CurrentIndex
					stack.State.PreviousLen = t.State.CurrentLen
				}
	...
	resultCh <- workerResult[*COPPStack]{w.id, stack}
}

func (w *coppWorker[T]) match(rhs []TokenType, rhsTokens []*CToken, isPrefix bool) (*CToken, error) {
	lhs, ruleNum := w.p.g.findMatch(rhs)
	if lhs == TokenEmpty {
	...
	ruleType := w.p.g.Rules[ruleNum].Type
...
func (w *coppWorker[T]) getNonterminal(rhsTokens []*CToken) *CToken {
