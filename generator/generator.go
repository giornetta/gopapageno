...
func run() error {
	...
	r := gopapageno.NewRunner(
		NewLexer(),
		NewGrammar(),
		gopapageno.WithConcurrency[%s](*concurrencyFlag),
		gopapageno.WithLogging[%s](log.New(logOut, "", 0)),
		gopapageno.WithCPUProfiling[%s](cpuProfileWriter),
		gopapageno.WithMemoryProfiling[%s](memProfileWriter),
		gopapageno.WithReductionStrategy[%s](strat),
		gopapageno.WithAverageTokenLength[%s](*avgTokensFlag),
	)
		...
		for c := 1; c <= threads; c = min(c*2, threads) {
			b.Run(fmt.Sprintf("%%s/%%dT", filename, c), func(b *testing.B) {
				r := gopapageno.NewRunner(
					NewLexer(),
					NewGrammar(),
					gopapageno.WithConcurrency[%s](c),
					gopapageno.WithReductionStrategy[%s](gopapageno.ReductionSweep))
...
func TestProfile(t *testing.T) {
	...
	r := gopapageno.NewRunner(
		...
		NewGrammar(),
		gopapageno.WithConcurrency[%s](c),
		gopapageno.WithAverageTokenLength[%s](avgLen),
		gopapageno.WithReductionStrategy[%s](strat),
	)
